...
def test_dosen_permissions():
    ...
    assert check_permission("dosen", "view_mk") is True
...
def test_admin_permissions():
    assert check_permission("admin_universitas", "view_all") is True
...
def test_unknown_role():
    assert check_permission("nonexistent", "view_all") is False
    assert get_scope("nonexistent") is None
    assert get_allowed_actions("nonexistent") == set()
